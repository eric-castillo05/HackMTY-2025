...
class model1Service:
    ...
    def add_calendar_feats(self, ddf: pd.DataFrame, date_col: str = COL_DATE) -> pd.DataFrame:
        ddf = ddf.copy()
    ...
    def ensure_required_columns(self, df: pd.DataFrame, required: List[str]) -> Optional[List[str]]:
        missing = [c for c in required if c not in df.columns]
        return missing if len(missing) > 0 else None

    def med_lastk(self, df_base: pd.DataFrame, mask, col: str, k: int = 28) -> Optional[float]:
